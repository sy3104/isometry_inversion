function [maxp S F]=maxp_isometry_inversion_sod_yalmip(d, D, Jin,k,choose_protocol,isComplex)
...
if k==1
    ...
    if isComplex==1
        S=sdpvar(din*d*D*dout_S,din*d*D*dout_S,'hermitian','complex');
        F=sdpvar(din*d*D*dout_F,din*d*D*dout_F,'hermitian','complex');
    else
        S=sdpvar(din*d*D*dout_S,din*d*D*dout_S,'symmetric','real');
        F=sdpvar(din*d*D*dout_F,din*d*D*dout_F,'symmetric','real');
    end
...
% This is made exploiting the Choi isomorphism
    for i=1:n_in
        ...
        Constraints = [Constraints, PartialTrace(PartialTranspose(Tensor(Jin(:,:,i), eye(dout_S)), [2], [d D dout_S])*Tensor(eye(d), Jout_S), [2], [d D dout_S])== p*max_ent_times_d];
        Constraints = [Constraints, PartialTrace(PartialTranspose(Tensor(Jin(:,:,i), eye(dout_F)), [2], [d D dout_F])*Tensor(eye(d), Jout_F), [2], [d D dout_F])== (1-p)*Jin(:,:,i)];
    end
end %end if k==1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% case of 2 copies
if k==2
    ...
    % Declare the SDP variables related to S and F
    if isComplex==1
        S=sdpvar(din*d*D*d*D*dout_S,din*d*D*d*D*dout_S,'hermitian','complex');
        F=sdpvar(din*d*D*d*D*dout_F,din*d*D*d*D*dout_F,'hermitian','complex');
    else
        S=sdpvar(din*d*D*d*D*dout_S,din*d*D*d*D*dout_S,'symmetric','real');
        F=sdpvar(din*d*D*d*D*dout_F,din*d*D*d*D*dout_F,'symmetric','real');
    end
...
if k==3
    ...
    % Declare the SDP variables related to S and F
    if isComplex==1
        S=sdpvar(din*d*D*d*D*d*D*dout_S,din*d*D*d*D*d*D*dout_S,'hermitian','complex');
        F=sdpvar(din*d*D*d*D*d*D*dout_F,din*d*D*d*D*d*D*dout_F,'hermitian','complex');
    else
        S=sdpvar(din*d*D*d*D*d*D*dout_S,din*d*D*d*D*d*D*dout_S,'symmetric','real');
        F=sdpvar(din*d*D*d*D*d*D*dout_F,din*d*D*d*D*d*D*dout_F,'symmetric','real');
    end
...
if choose_protocol==1
    Constraints = is_parallel_protocol_yalmip(C,din,1,d,D,k,Constraints);
end
if choose_protocol==2
    Constraints = is_sequential_protocol_yalmip(C,din,1,d,D,k,Constraints);
end
if choose_protocol==3
    Constraints = is_general_protocol_yalmip(C,din,1,d,D,k,Constraints);
end
