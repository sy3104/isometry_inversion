function new_Constraints = is_parallel_protocol_yalmip(C,din,dout,d,D,k,Constraints)
...
%Impose the condition that C is a superchannel that makes a single use of the input-operation
    new_Constraints=[Constraints, PartialTrace(C,4,[d1 d2 d3 d4])==kron( PartialTrace(C,[3 4],[d1 d2 d3 d4]),eye(d3)/d3)];
    new_Constraints=[new_Constraints, PartialTrace(C,[2 3 4],[d1 d2 d3 d4])==eye(d1)*d3];
end
