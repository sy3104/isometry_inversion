...
%'D'-dimensional space into its inverse map in "success-or-draw" manner.
%Please set the parameters 'k','d','D','n','choose protocol', and 'isComplex'
%The interpreter cvx is used to conduct SDP.


%Set the dimension d and D of the isometry that will be inverted
...
%Set isComplex==1 to consider general complex valued supermaps
        ...
        n=100;               %Number of linear independent V^k, n==0 implies full basis
        choose_protocol=2; %1 for parallel, 2 for sequential, 3 for general
...
if n==0
% Set 'n' as the dimension of the space spanned by k+1 isometry operations
% from d-dimensional space to D-dimensional space.
    n=find_dimension_isometry(d,D,k+1)
else
    n=n
        if find_dimension_isometry(d,D,k+1)>n
            'The dimension of the space spanned by V^k+1 is smaller than the number of isometries n'
...
%Declare variables that contain the Choi input operations
    Jin=zeros(d*D,d*D,n);  
    list_V=zeros(D,d,n);
%Asign values for Choi inputs
for i=1:n 
...
%d-dimensional space to D-dimensional space into its inverse map
%in "success-or-draw" manner
[maxp S F]=maxp_isometry_inversion_sod_cvx(d, D, Jin, k, choose_protocol, isComplex);
